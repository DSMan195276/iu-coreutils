...
#define list_foreach_take_entry(head, pos, member)                                  \
    for (pos = list_empty(head)? NULL: list_take_first(head, __typeof__(*pos), member); \
         pos;                                                                       \
         pos = list_empty(head)? NULL: list_take_first(head, __typeof__(*pos), member))

#define list_foreach_take_entry_reverse(head, pos, member) \
    for (pos = list_empty(head)? NULL: list_take_last(head, __typeof__(*pos), member); \
         &(pos)->member != NULL;                           \
         pos = list_empty(head)? NULL: list_take_last(head, __typeof__(*pos), member))
